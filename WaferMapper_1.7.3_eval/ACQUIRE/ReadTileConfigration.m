...
while true
    ...
    TileConfiguration.TileArray(RowNum, ColNum).FullPathName = sprintf('%s\\%s',MontageDir, JustFileName);
...
for RowNum = 1:MaxRow
    for ColNum = 1:MaxCol
        ...
        if ColNum == 1
            ...
            TileConfiguration.TileArray(RowNum, ColNum).MinX = (CenterX + TileConfiguration.TileArray(RowNum, ColNum-1).XCoord)/2;
        end
        
        if ColNum == MaxCol
            TileConfiguration.TileArray(RowNum, ColNum).MaxX = CenterX + TileConfiguration.TileWidthInPixels/2;
        else
            TileConfiguration.TileArray(RowNum, ColNum).MaxX = (CenterX + TileConfiguration.TileArray(RowNum, ColNum+1).XCoord)/2;
        end
        
        if RowNum == 1
            TileConfiguration.TileArray(RowNum, ColNum).MinY = CenterY - TileConfiguration.TileHeightInPixels/2;
        else
            TileConfiguration.TileArray(RowNum, ColNum).MinY = (CenterY + TileConfiguration.TileArray(RowNum-1, ColNum).YCoord)/2;
        end
        
        if RowNum == MaxRow
            TileConfiguration.TileArray(RowNum, ColNum).MaxY = CenterY + TileConfiguration.TileHeightInPixels/2;
        else
            TileConfiguration.TileArray(RowNum, ColNum).MaxY = (CenterY + TileConfiguration.TileArray(RowNum+1, ColNum).YCoord)/2;
        end
