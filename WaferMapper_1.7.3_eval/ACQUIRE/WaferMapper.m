...
% Last Modified by GUIDE v2.5 21-Nov-2012 15:08:19
...
if ~exist(GuiGlobalsStruct.UTSLDirectory, 'dir')
    ...
    DirListArray = dir(GuiGlobalsStruct.UTSLDirectory); DirListArray = DirListArray(3:end);
    
    ListOfDirNames = [];
    for i=1:length(DirListArray)
        if (DirListArray(i).isdir == 1)
            nam = DirListArray(i).name;
            if strcmp(lower(nam(1)),'w')
                ListOfDirNames{length(ListOfDirNames)+1} = nam;
            end
        end
    end
    ...
    if length(ListOfDirNames) > 0
        ...
        if isok == 1
            ...
            GuiGlobalsStruct.WaferNameIndex = SelectionNumber;
...
InputDlgAnswer = inputdlg('Enter name for new Wafer (must start with ''w'' )');
if isempty(InputDlgAnswer)
    disp('User Cancled');
    return;
else
...
if exist(GuiGlobalsStruct.LowResFiducialsDirectory, 'dir')
    MyStr = sprintf('%s already exists. If you are sure you want to acquire new low res fiducials for this wafer then manually delete the directory.',...
...
if exist(GuiGlobalsStruct.HighResFiducialsDirectory, 'dir')
    MyStr = sprintf('%s already exists. If you are sure you want to acquire new high res fiducials for this wafer then manually delete the directory.',...
...
if exist(GuiGlobalsStruct.ExampleSectionImageDirectory, 'dir')
    MyStr = sprintf('%s already exists. If you are sure you want to acquire a new example image for this wafer then manually delete the directory.',...
...
% --------------------------------------------------------------------
function ChooseSectionTemplateImageAllWafers_MenuItem_Callback(hObject, eventdata, handles)
% hObject    handle to ChooseSectionTemplateImageAllWafers_MenuItem (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
uiwait(msgbox('NOTE: Pick template for each wafer. Press OK to choose.'));
...
SectionOverviewTemplateDirectory = sprintf('%s\\%s\\SectionOverviewTemplateDirectory',GuiGlobalsStruct.UTSLDirectory,waferName);

if ~exist(SectionOverviewTemplateDirectory, 'dir')
    [success,message,messageid] = mkdir(GuiGlobalsStruct.SectionOverviewTemplateDirectory);
end

SODir = sprintf('%s\\%s\\SectionOverviewsDirectory',GuiGlobalsStruct.UTSLDirectory,waferName);

if exist([SODir  'Filtered'],'dir')
    SODir = [SODir 'Filtered'];
end
...
if length(ListOfTifFileNames) > 0
    [SelectionNumber,isok] = listdlg('PromptString',waferName,...
        'SelectionMode','single',...
        'ListString',ListOfTifFileNames);
...
h_fig = figure();
MyImage = imread(ImageFileNameStr);
imshow(MyImage, [0,255]); pause(.5)

NewImageFileNameStr = sprintf('%s\\SectionOverviewTemplate.tif',SectionOverviewTemplateDirectory); 
%MyStr = sprintf('About to copy file: %s to new file: %s',ImageFileNameStr,NewImageFileNameStr);
%ButtonName = questdlg(MyStr,'title','OK','Cancel','OK');
if exist(NewImageFileNameStr)
    movefile(NewImageFileNameStr,sprintf('%s\\SectionOverviewTemplate%s.tif',SectionOverviewTemplateDirectory,num2str(datenum(clock))));
    imwrite(MyImage,NewImageFileNameStr,'tif');
end
...
set(handles.ChooseSectionTemplateImageAllWafers_MenuItem,'Checked','on');
set(handles.CropSectionTemplateImage_MenuItem,'Enable','on');
...
AlignOverviews(GuiGlobalsStruct.WaferName);
toc(start)
set(handles.AlignSectionOverviews_MenuItem,'Checked','on');
...
% --------------------------------------------------------------------
function AlignSectionOverviewsAllWafers_MenuItem_Callback(hObject, eventdata, handles)
% hObject    handle to AlignSectionOverviewsAllWafers_MenuItem (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
for w = 3:length(waferList)
    
    waferName = waferList{w};
    disp(sprintf('Aligning Wafer %s.',waferName))
    AlignOverviews(waferName);
...
CropSectionOverviewTemplate(GuiGlobalsStruct.SectionOverviewTemplateDirectory);
set(handles.CropSectionTemplateImage_MenuItem,'Checked','on');
set(handles.AlignSectionOverviews_MenuItem,'Enable','on');

% --------------------------------------------------------------------
function CropSectionTemplateImageAllWafers_MenuItem_Callback(hObject, eventdata, handles)
% hObject    handle to CropSectionTemplateImageAllWafers_MenuItem (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
%Check that directory and SectionOverviewTemplate.tif exist
SectionOverviewTemplateFileNameStr = sprintf('%s\\SectionOverviewTemplate.tif',SectionOverviewTemplateDirectory);

if ~exist(SectionOverviewTemplateDirectory, 'dir')
    MyStr = sprintf('%s does not exist.',...
        SectionOverviewTemplateDirectory);
    uiwait(msgbox(MyStr,'modal'));
    return;
end
if ~exist(SectionOverviewTemplateFileNameStr, 'file')
...
CropSectionOverviewTemplate(SectionOverviewTemplateDirectory);
end % if template file exists
end
...
set(handles.CropSectionTemplateImageAllWafers_MenuItem,'Checked','on');
set(handles.AlignSectionOverviews_MenuItem,'Enable','on');
...
if ~( isnan(MyAnswer2Num) || (MyAnswer2Num < 0) || (MyAnswer2Num > 360) )
    ...
    uiwait(msgbox('Ilegal value. Not updating.')) ;
end
...
if ~exist(GuiGlobalsStruct.AlignedTargetListsDirectory, 'dir')
    ...
    if length(ListOfDirNames) > 0
        ...
        if isok == 1
            ...
            %Use first section's r,c point as target (these should all be
            %the same anyway)
            %uses the first non empty entry (needed when doing more wafers)
%             for WaferNameIndex = 1:length(AlignedTargetList.WaferArray)
%                 if ~isempty(AlignedTargetList.WaferArray(1).SectionArray)
%                     break;
%                 end
%             end
            
            %WaferNameIndex = 1; %%%change back to 1
            %find a non empty wafer in the aligned target list
            for WaferNameIndex = 1:length(AlignedTargetList.WaferArray)
                if ~isempty(AlignedTargetList.WaferArray(WaferNameIndex).SectionArray)
                    break;
                end
            end
            GuiGlobals.Struct.WaferNameIndex = WaferNameIndex
            SectionIndex = 1;
...
for i = 1:length(GuiGlobalsStruct.ListOfWaferNames)
    ...
    if strcmp(GuiGlobalsStruct.WaferDirectory, WaferDirName)
        WaferNameIndex = i;
       GuiGlobalsStruct.WaferNameIndex = i; 
...
WaferName_CurrentlyLoaded = GuiGlobalsStruct.ListOfWaferNames{WaferNameIndex};
...
% --------------------------------------------------------------------
function AverageAllAlignedOverviews_Callback(hObject, eventdata, handles)
% hObject    handle to AverageAllAlignedOverviews (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global GuiGlobalsStruct

UTSLDir = GuiGlobalsStruct.UTSLDirectory;
WaferNames = GuiGlobalsStruct.ListOfWaferNames;
for i = 1:length(WaferNames)
    
    
    soDir = [UTSLDir '\' WaferNames{i} '\' 'SectionOverviewsAlignedWithTemplateDirectory\'];
    
    if exist(soDir,'dir')
        targetDirectory =  [ UTSLDir '\' WaferNames{i} '\SectionOverviewTemplateDirectory\'];
        if 1%~exist([targetDirectory 'averageAlignedOverviews.tif']);
            
            soNams = dir(soDir); soNams = soNams(3:end);
            clear stackI
            countI = 0;
            for n = 1:length(soNams)
                nam = soNams(n).name;
                if strcmp(nam(end-3:end),'.tif');
                    ...
                    if ~exist('stackI')
                        ...
                        stackI = stackI + double(I);
                        disp(sprintf('Finished %s from %s',nam,WaferNames{i}))
                        
                    end %if first section
                    
                end % if tif
                
            end % run all images
            
            meanI = uint8(stackI/countI);
            
            if ~exist(targetDirectory)
                mkdir(targetDirectory)
            end
            imwrite(meanI,[targetDirectory 'averageAlignedOverviews.tif'],'Compression','none');
            
            
        end % if mean image doesnt exist 
        end % if aligned overviews exist
...
% --------------------------------------------------------------------
function AverageAlignedOverviews_Callback(hObject, eventdata, handles)
% hObject    handle to AverageAlignedOverviews (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
global GuiGlobalsStruct
    ...
    soDir = [UTSLDir '\' GuiGlobalsStruct.WaferName '\' 'SectionOverviewsAlignedWithTemplateDirectory\'];
    
    if exist(soDir,'dir')
        targetDirectory =  [ UTSLDir '\' GuiGlobalsStruct.WaferName '\SectionOverviewTemplateDirectory\'];
            ...
            for n = 1:length(soNams)
                ...
                if strcmp(nam(end-3:end),'.tif');
                    ...
                    if ~exist('stackI')
                        ...
                        stackI = stackI + double(I);
                        disp(sprintf('Finished %s from %s',nam,GuiGlobalsStruct.WaferName))
                    end %if first section
            ...
            meanI = uint8(stackI * 255/max(stackI(:)));
...
% --------------------------------------------------------------------
function AlignWafers_Manual_Callback(hObject, eventdata, handles)
% hObject    handle to AlignWafers_Manual (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --------------------------------------------------------------------
function WaferAlignmentMenu_Callback(hObject, eventdata, handles)
% hObject    handle to WaferAlignmentMenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)


% --------------------------------------------------------------------
function AlignWafers_Automatic_Callback(hObject, eventdata, handles)
% hObject    handle to AlignWafers_Automatic (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
AlignWafers_Automatic()
