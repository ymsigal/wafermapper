...
TPN = 'D:\LGNs1\rawMontages_folder2\'; % local intermediate buffer drive

%%  Select actions
ClearScope = 1;  %Copy then delete finished sections
CopyScope = 1;  %Copy all data from scope to buffer
CopyBuffer = 0; %Copy data from buffer to server
    ...
    if ClearScope
        ...
        if ~isempty(finishedFolders)
            for s = 1: min(length(finishedFolders),checkSecNumber)
                ...
                %%Make folder
                if ~exist([TPN nam],'dir')
                    mkdir([TPN nam])
                end
                tic
                %%Copy Folder
                allGood = shortPush([SPN nam '\'],[TPN nam '\']);
                disp('section copy finished')
                toc
                %%Double Check Folder
                allGood = compFolders([SPN nam '\'],[TPN nam '\']);
...
%                     md5List = checkSumFolder([TPN nam '\']);
%                     safesave([TPN nam '\md5s.mat'],'md5List')
%                     toc
    ...
    if CopyScope
        ...
        pushFolders(SPN,TPN,delaySecCheckDays)
        %shortPush(SPN,TPN);
    end
...
%         ignoreMemory = 1;
%         pushFolders(TPN,RCPN,delaySecCheckDays,ignoreMemory)
%     end
