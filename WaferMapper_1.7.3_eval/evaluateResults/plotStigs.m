...
% logFile = 'D:\LGNs1\rawMontages\logBooks\LogBook_w032.mat';
% 
% load(logFile);
% pause(1)    
% load(logFile);
...
stigX = cat(1,logBook.sheets.imageConditions.data{:,11});
stigY = cat(1,logBook.sheets.imageConditions.data{:,12});
WD = cat(1,logBook.sheets.imageConditions.data{:,5});


scaleWD = WD-mean(WD);
scaleWD = scaleWD/std(scaleWD);
hist(scaleWD)
...
scaleQual = tileQual-mean(tileQual);
scaleQual = scaleQual/std(scaleQual);
plot(scaleQual,'m')
