...
%% Create matrix
subplot(1,1,1)
globalY = max(trackY(1,:))+10;
globalX = max(trackX(:,1))+10;
realWidth = round((W- (W * overlap/100)));
maxX = fix((W-grabOverlap) * numCol - min(sum(trackX,2))+globalX+grabOverlap)+10;
maxY = fix((H-grabOverlap) * numRow - min(sum(trackY,1))+globalY+grabOverlap)+10;
stitched = zeros(maxY, maxX,3);
...
stitchStats.meanXerror = sum(sum(abs([leftCor(:,:,2) upCor(:,:,2)])))/(numRow*numCol*2-1);
stitchStats.meanYerror = sum(sum(abs([leftCor(:,:,1) upCor(:,:,1)])))/(numRow*numCol*2-1);
