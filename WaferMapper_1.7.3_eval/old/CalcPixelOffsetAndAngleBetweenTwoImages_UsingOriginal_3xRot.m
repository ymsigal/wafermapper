...
OriginalImage_3xSizeWithFilledInBackground = uint8(zeros(3*HeightOriginal,3*WidthOriginal));
AvgOrigBackground = median(OriginalImage(:));
OriginalImage_3xSizeWithFilledInBackground(:,:) =  AvgOrigBackground;
OriginalImage_3xSizeWithFilledInBackground(HeightOriginal+1:(end-HeightOriginal), WidthOriginal+1:(end-WidthOriginal)) =...
  OriginalImage;
...
for i = 1:length(AnglesInDegreesToTryArray)
    ...
    OriginalImage_3Xrotated = imrotate(OriginalImage_3xSizeWithFilledInBackground,AnglesInDegreesToTryArray(i),'crop');
    OriginalImage_rotated = OriginalImage_3Xrotated(HeightOriginal+1:2*HeightOriginal,WidthOriginal+1:2*WidthOriginal);
