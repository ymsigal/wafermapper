...
if strcmp(AnswerName, 'Yes')
    ...
    if isnan(MyAnswer2Num) %if NaN then use stage coordinates
        TopStageY_mm = floor(1000*GuiGlobalsStruct.MyCZEMAPIClass.Get_ReturnTypeSingle('AP_STAGE_AT_Y')); %round down to nearest mm
    ...
    if isnan(MyAnswer2Num) %if NaN then use stage coordinates
        BottomStageY_mm = floor(1000*GuiGlobalsStruct.MyCZEMAPIClass.Get_ReturnTypeSingle('AP_STAGE_AT_Y')); %round down to nearest mm
    ...
    if isnan(MyAnswer2Num) %if NaN then use stage coordinates
        LeftStageX_mm = floor(1000*GuiGlobalsStruct.MyCZEMAPIClass.Get_ReturnTypeSingle('AP_STAGE_AT_X')); %round down to nearest mm
    ...
    if isnan(MyAnswer2Num) %if NaN then use stage coordinates
        RightStageX_mm = floor(1000*GuiGlobalsStruct.MyCZEMAPIClass.Get_ReturnTypeSingle('AP_STAGE_AT_X')); %round down to nearest mm
