function[] = AlignOverviews(waferName)
%AlignOverviews
...
if exist(SectionOverviewsAlignedWithTemplateDirectory)
    backupSOAWTDirectory = [SectionOverviewsAlignedWithTemplateDirectory num2str(datenum(clock))];
    movefile(SectionOverviewsAlignedWithTemplateDirectory,backupSOAWTDirectory);
    [success,message,messageid] = mkdir(SectionOverviewsAlignedWithTemplateDirectory);
...
FileNameStr = sprintf('%s\\SectionOverviewTemplateCroppedFilledPeriphery.tif',SectionOverviewTemplateDirectory);
...
MaxLabelNum = -1;
DirList = dir(SectionOverviewsDirectory);
...
disp( sprintf('About to process %d sections.', MaxLabelNum));
...
for i = 1:MaxLabelNum
    disp(sprintf('Aligning section %d:', i));
    ...
    OverviewImageFileNameStr = sprintf('%s\\SectionOverview_%s.tif',SectionOverviewsDirectory,Label);
    ...
    OverviewImageAlignedFileNameStr = sprintf('%s\\SectionOverviewAligned_%s.tif',SectionOverviewsAlignedWithTemplateDirectory,Label);
    OverviewAlignedDataFileNameStr = sprintf('%s\\SectionOverviewAligned_%s.mat',SectionOverviewsAlignedWithTemplateDirectory,Label);
