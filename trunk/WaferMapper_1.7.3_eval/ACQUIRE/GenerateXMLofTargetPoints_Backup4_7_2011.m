...
for SectionIndex = 1:length(GuiGlobalsStruct.AlignedTargetList.WaferArray(WaferNameIndex).SectionArray)
    ...
    if IsTakeTestImage
        ...
        NewScanRot = TempStoreScanRot + GuiGlobalsStruct.MontageTarget.MontageNorthAngle;
        %NOTE: YOU NEED TO CHECK IF BETWEEN 0 and 360 and correct here
        if NewScanRot > 360
        ...
        ImageFileNameStr = sprintf('%s\\TestImage_%s.tif', GuiGlobalsStruct.TempImagesDirectory, MySection.LabelStr);
        ...
        DwellTimeInMicroseconds = 2;
        FOV_microns = GuiGlobalsStruct.MontageTarget.MontageWidthInMicrons; %40.96;
