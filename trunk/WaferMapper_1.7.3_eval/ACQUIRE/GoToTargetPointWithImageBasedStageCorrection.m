...
if exist('DoAutoFocusAfterGoToMontageTargetMove','var')
    if DoAutoFocusAfterGoToMontageTargetMove
        ...
        pause(0.5);
        if abs(GuiGlobalsStruct.WD_AtAcquitionStart - ResultingWorkingDistance) > 0.0005 %0.5mm
...
%START: JOSH'S FILTER
[OriginalImage_x3LargerROI_Filtered] = mexHatVessle(OriginalImage_x3LargerROI_Filtered);
OriginalImage_x3LargerROI_Filtered = 255*(OriginalImage_x3LargerROI_Filtered/max(max(OriginalImage_x3LargerROI_Filtered)));
[CurrentImage_Filtered] = mexHatVessle(CurrentImage_Filtered);
...
if ~goodMatch
    disp(sprintf('********!!! IBSC faild to find match. Reinitializing stage ********'));
