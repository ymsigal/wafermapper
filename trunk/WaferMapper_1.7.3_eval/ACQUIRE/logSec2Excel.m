function[logBook] = logSec2Excel(logBook,bookDir,bookName)
...
safesave([GuiGlobalsStruct.TempImagesDirectory '\logBooks\' bookName '.mat'],'logBook')
    ...
    [sorted sortStart] = sort(startTimeNum,'descend') ;
    ...
    clear secDat
    minSec = min([length(listSecs)  length(startTime) length(midQual) ...
        length(secDuration) length(midCurrent) length(offSets) length(offAngle) length(secMerit)]);
    sortStart = sortStart(sortStart<=minSec);
    for i = 1:(sortStart)
        secDat{i,1} = listSecs(sortStart(i));
        secDat{i,2} = startTime{sortStart(i)};
        secDat{i,3} = midQual(sortStart(i));
        secDat{i,4} = minQual(sortStart(i));
        secDat{i,5} = secDuration(sortStart(i));
        secDat{i,6} = midCurrent(sortStart(i));
        secDat{i,7} = offSets(sortStart(i));
        secDat{i,8} = offAngle(sortStart(i));
        secDat{i,9} = secMerit(sortStart(i));
    end
    
    
    
    secHeader = {'Section' 'Start' 'midQuality' 'minQual' 'Duration' 'Current' 'IBSC xy' 'IBSC angle' 'IBSC merit'};
    
    logBook.sheets.secSum.header = secHeader;
    logBook.sheets.secSum.data = secDat;
    
    %save([gDrive bookName '.mat'],'logBook');
    ...
    %% Set up excel
%     
%     bookFileNameXLS =[GuiGlobalsStruct.TempImagesDirectory '\logBooks\' bookName '.xls'];
%     
