function [XOffsetOfNewInPixels, YOffsetOfNewInPixels, AngleOffsetOfNewInDegrees, FigureOfMerit] = CalcPixelOffsetAndAngleBetweenTwoImages_FurtherRestrictToCenter(OriginalImage, NewImage,OriginalImage_x3LargerROI, AnglesInDegreesToTryArray)
...
for i = 1:length(AnglesInDegreesToTryArray)
    ...
    if IsRestrictToCenter
        %Restrict to center:
%         C_ValidRegion(1:floor(end/3), :) = 0; %original code
%         C_ValidRegion(2*floor(end/3):end, :) = 0;
%         C_ValidRegion(:, 1:floor(end/3)) = 0;
%         C_ValidRegion(:, 2*floor(end/3):end) = 0;
        
        %Restrict to center:
        C_ValidRegion(1:floor(4*end/9), :) = 0;
        C_ValidRegion(floor(5*end/9):end, :) = 0;
        C_ValidRegion(:, 1:floor(4*end/9)) = 0;
        C_ValidRegion(:, floor(5*end/9):end) = 0;
    end
