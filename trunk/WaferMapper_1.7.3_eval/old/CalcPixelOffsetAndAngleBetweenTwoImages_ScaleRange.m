function [XOffsetOfNewInPixels, YOffsetOfNewInPixels, AngleOffsetOfNewInDegrees] = CalcPixelOffsetAndAngleBetweenTwoImages_ScaleRange(OriginalImage, NewImage, AnglesInDegreesToTryArray,ScalesToTryArray)
...
MaxSoFar = -1000000000;
for j = 1:length(ScalesToTryArray)
    
    NewImage_Scaled = imresize(NewImage, ScalesToTryArray(j), 'bilinear');
    [WidthNewImage, dummy] = size(NewImage_Scaled);
    [WidthImage, HeightImage] = size(NewImage_Scaled);
    ...
    for i = 1:length(AnglesInDegreesToTryArray)
        ...
        if IsPlotResults
            ...
            ColorCombinedImage(HeightImage+1:(end-HeightImage), WidthImage+1:(end-WidthImage),1) = NewImage_Scaled; %red (centered template)
        ...
        C = normxcorr2(NewImage_scaled, OriginalImage_3xSizeWithFilledInBackground_rotated);
        ...
        if IsPlotResults
            ...
            ColorCombinedImage(HeightImage+1+r_offset:(end-HeightImage)+r_offset,...
                WidthImage+1+c_offset:(end-WidthImage)+c_offset,1) = NewImage_Scaled;
...
if IsPlotResults
    ...
    NewImage_Scaled = imresize(NewImage, ScalesToTryArray(j_final), 'bilinear');
    [WidthNewImage, dummy] = size(NewImage_Scaled);
    [WidthImage, HeightImage] = size(NewImage_Scaled);
